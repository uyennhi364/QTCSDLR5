...
namespace WindowsFormsAppQLBH_MON
{
    ...
    public partial class FormQLBH_MON : Form
    {
        ...
        {
            string tuKhoa = text_TENMON.Text.Trim(); // Tên món cần tìm

            if (string.IsNullOrEmpty(tuKhoa))
            {
                LoadData(); // Nếu không nhập, hiển thị lại toàn bộ danh sách
                return;
            }
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = @"
                SELECT MAMON, TENMON, GIABAN, DONVI
                FROM MON
                WHERE TENMON LIKE @tuKhoa";
                    ...
                    {
                        MessageBox.Show("Không tìm thấy món nào phù hợp!");
                    }
                }
                catch (Exception ex)
                {
        ...
        private void ClearForm()
        {
            ...
            text_GIABAN.Clear();
            text_DONVI.Clear();
            text_MAMON.Focus(); // Đưa con trỏ về ô nhập đầu tiên

            selectedRowIndex = -1; // Nếu bạn dùng biến này để đánh dấu dòng được chọn
        }
