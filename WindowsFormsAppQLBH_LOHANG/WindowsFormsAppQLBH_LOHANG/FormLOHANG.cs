...
namespace WindowsFormsAppQLBH_LOHANG
{
    public partial class FormLOHANG : Form
    {
        ...
        {
            InitializeComponent();
            // Gán sự kiện click cho nút Thông báo HSD
            btn_ThongBao.Click += (s, e) => ThongBao_HSD_LOHANG();
        ...
        {
            LoadData();
            DataGridView_ThongBaoHetHan.Visible = false;
        }
        ...
        private void ThongBao_HSD_LOHANG()
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand("ThongBao_HSD_LOHANG", conn);
                ...
                try
                {
                    conn.Open();
                    adapter.Fill(dt);
                    DataGridView_ThongBaoHetHan.DataSource = dt;

                    if (dt.Rows.Count == 0)
                    {
                        DataGridView_ThongBaoHetHan.Visible = false;
                        MessageBox.Show("Không có lô hàng nào sắp hết hạn trong 10 ngày tới.", "Thông báo");
                    }
                    else
                    {
                        DataGridView_ThongBaoHetHan.Visible = true;
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Lỗi khi truy vấn Thông báo HSD: " + ex.Message);
                }
            }
        }
        private void ClearForm()
        {
            ...
            txt_MALH.Focus();
            selectedRowIndex = -1; // nếu bạn dùng biến này để theo dõi dòng được chọn
        }
